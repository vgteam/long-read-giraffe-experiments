...
def main(args):
    ...
    if use_series:
        ...
        if options.y_per_category:
            ...
            series_plots = [pyplot] * len(category_order)
    ...
    for series_plot, series_name, series_color, series_symbol in \
        ...
        plot_func = series_plot.plot if options.lines else series_plot.scatter
        ...
        # Do the actual plot
        plotted_items.append(plot_func(series[series_name][0],
    ...
    for plot, min_y in zip(series_plots, options.min_y):
        ...
        plot.set_ylim((options.min_y, plot.get_ylim()[1]))    
    for plot, min_y in zip(series_plots, options.min_y):
        # Set the maximum on each Y axis
        plot.set_ylim((plot.get_ylim()[0], options.max_y))
