...
srun -c16 --mem 300G --job-name giraffe-run vg giraffe --parameter-preset sr --progress --track-provenance -Z ${GRAPH_BASE}.gbz -d ${GRAPH_BASE}.dist -m ${GRAPH_BASE}.${MINPARAMS}.withzip.min -z ${GRAPH_BASE}.${MINPARAMS}.zipcodes -G ${INPUT_READS} --${OPTION_NAME} ${OPTION_RANGE} --output-basename ${SEARCH_DIR}/search

for GAM_FILE in ${SEARCH_DIR}/*.gam ; do
    if [[ ! -e "${GAM_FILE%.gam}.compare.txt" ]] ; then
        sbatch -c4 --mem 25G --job-name giraffe-eval --wrap "vg annotate -a ${GAM_FILE} -x ${GRAPH_BASE}.gbz -m | vg gamcompare --range 200 - ${INPUT_READS} -T -a "${GAM_FILE}" > ${GAM_FILE%.gam}.compared.tsv 2>${GAM_FILE%.gam}.compare.txt"
    fi
    if [[ ! -e "${GAM_FILE%.gam}.gamstats.txt" ]] ; then
        sbatch -c2 --mem 10G --job-name giraffe-stats --wrap "vg stats -a "${GAM_FILE}" >${GAM_FILE%.gam}.gamstats.txt"
    fi
done
...
# Plot PR and QQ
srun -c2 --mem 10G Rscript scripts/plot-pr.R "${COMPARISON_SCRATCH}" ${WORK_DIR}/${OPTION_NAME}.compared.png
srun -c2 --mem 10G Rscript scripts/plot-qq.R "${COMPARISON_SCRATCH}" ${WORK_DIR}/${OPTION_NAME}.qq.png

# Dump QC stats and speed
rm ${WORK_DIR}/${OPTION_NAME}.stats.txt
for GAM_FILE in ${SEARCH_DIR}/*.gam ; do
