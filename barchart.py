...
def main(args):
    ...
    if options.categories is not None:
        ...
        if options.ascending is True:
            ...
            if options.colors is not None:
                color_map = dict()
                for i in range(len(options.categories)):
                    color = matplotlib.colors_to_rgb(options.colors[i]) if options.colors[i][0] == '#' else options.colors[i]
        ...
        category_order = sorted(categories.keys(), key=natural_keys)
    ...
    for i in range(len(category_order)):
        ...
        if not category_order[i] in categories:
